...
function AdminControlCenter({
  ...
  const [manageView, setManageView] = useState("employees");
  const [selectedProfileId, setSelectedProfileId] = useState(null);
  const [selectedEmployeeJobKey, setSelectedEmployeeJobKey] = useState(null);
  ...
  const selectedEmployeeJob = employeeJobs.find((job) => job.key === selectedEmployeeJobKey) || employeeJobs[0] || null;
    ...
    <Card className="manage-admin-shell overflow-hidden">
      <CardContent className="p-0">
        ...
        <div className="space-y-5 p-4 sm:p-5">
          <div className="manage-view-switch grid grid-cols-2 gap-2 rounded-[1.35rem] border border-slate-200/80 bg-slate-50/75 p-1.5 dark:border-white/10 dark:bg-slate-950/30"><button type="button" onClick={() => setManageView("employees")} className={cx("rounded-[1rem] px-4 py-3 text-sm font-black transition", manageView === "employees" ? "bg-white text-slate-950 shadow-sm dark:bg-white dark:text-slate-950" : "text-slate-500 hover:text-slate-950 dark:text-slate-400 dark:hover:text-white")}>Employees</button><button type="button" onClick={() => setManageView("jobs")} className={cx("rounded-[1rem] px-4 py-3 text-sm font-black transition", manageView === "jobs" ? "bg-white text-slate-950 shadow-sm dark:bg-white dark:text-slate-950" : "text-slate-500 hover:text-slate-950 dark:text-slate-400 dark:hover:text-white")}>Master Jobs</button></div>

          {manageView === "employees" && selectedProfile && (
            <section className="employee-profile-panel rounded-[1.75rem] border border-cyan-200/70 bg-cyan-50/55 p-4 shadow-sm dark:border-cyan-300/15 dark:bg-cyan-400/[0.055] sm:p-5">
              ...
              <div className="mt-5"><div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Job history</p><h3 className="text-lg font-black">Select a job</h3></div><p className="text-xs font-bold text-slate-500 dark:text-slate-400">{employeeJobs.length} total jobs · all recorded history</p></div>{employeeJobs.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-200 bg-white/60 p-5 text-center text-sm font-bold text-slate-500 dark:border-white/10 dark:bg-white/5 dark:text-slate-400">No hours recorded for this employee yet.</div> : (<div className="employee-job-drilldown grid gap-4 lg:grid-cols-[minmax(230px,.82fr)_minmax(0,1.5fr)]"><div className="employee-job-picker space-y-2">{employeeJobs.map((job) => { const selected = selectedEmployeeJob?.key === job.key; return <button key={job.key} type="button" onClick={() => setSelectedEmployeeJobKey(job.key)} className={cx("employee-job-picker-button w-full rounded-2xl border p-3.5 text-left transition", selected ? "border-cyan-300 bg-cyan-50 shadow-sm dark:border-cyan-300/25 dark:bg-cyan-400/[0.08]" : "border-slate-200 bg-white/80 hover:border-cyan-200 hover:bg-white dark:border-white/10 dark:bg-slate-950/30 dark:hover:bg-white/[0.06]")}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="clean-wrap text-sm font-black text-slate-950 dark:text-white">{job.name}</p><p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400">{job.dates.size} day{job.dates.size === 1 ? "" : "s"} · {job.entries.length} entr{job.entries.length === 1 ? "y" : "ies"}</p></div><span className="shrink-0 rounded-full bg-cyan-50 px-2.5 py-1 text-xs font-black text-cyan-700 dark:bg-cyan-400/10 dark:text-cyan-200">{job.total.toFixed(2)}h</span></div></button>; })}</div>{selectedEmployeeJob && <div className="employee-job-detail rounded-[1.35rem] border border-slate-200 bg-white/90 p-4 dark:border-white/10 dark:bg-slate-950/35 sm:p-5"><div className="flex flex-col gap-2 border-b border-slate-100 pb-4 dark:border-white/10 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-700 dark:text-cyan-300">Selected job</p><h4 className="clean-wrap mt-1 text-xl font-black text-slate-950 dark:text-white">{selectedEmployeeJob.name}</h4><p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400">Complete work history for {selectedProfile.name}</p></div><span className="self-start rounded-full bg-cyan-50 px-3 py-1.5 text-sm font-black text-cyan-700 dark:bg-cyan-400/10 dark:text-cyan-200">{selectedEmployeeJob.total.toFixed(2)}h total</span></div><div className="mt-3 divide-y divide-slate-100 dark:divide-white/10">{selectedEmployeeJob.entries.map((entry) => <div key={entry.id} className="grid grid-cols-[1fr_auto] gap-3 py-3 text-sm"><div className="min-w-0"><p className="font-black text-slate-800 dark:text-slate-100">{displayDate(entry.date)}</p><p className="mt-1 clean-wrap text-xs font-semibold text-slate-500 dark:text-slate-400">{entry.jobType || "Job"}{entry.start && entry.end ? ` · ${entry.start}–${entry.end}` : ""}</p></div><span className="font-black tabular-nums text-slate-800 dark:text-slate-100">{entryHours(entry).toFixed(2)} hrs</span></div>)}</div></div>}</div>)}</div>
            </section>
          )}

          <div className="grid gap-5">
            {manageView === "employees" && <section className="manage-section-card rounded-[1.65rem] border border-slate-200/80 bg-white/75 p-4 shadow-sm dark:border-white/10 dark:bg-white/5 sm:p-5">
            ...
            {manageView === "jobs" && <section className="manage-section-card rounded-[1.65rem] border border-slate-200/80 bg-white/75 p-4 shadow-sm dark:border-white/10 dark:bg-white/5 sm:p-5">
...
export default function RestorationHoursTracker() {
  ...
  function exportPayrollPdf(employeeOverrideId = null) {
    ...
    </style></head><body><main class="sheet"><section class="hero"><span class="pill">VODA Of Tucson</span><h1>Hours Report</h1><p class="subtitle">Two-week pay period: ${displayShortDate(weekStart)} - ${displayShortDate(addDays(weekStart, 13))} • Submitted and approved hours • Phoenix time</p></section><section class="summary"><div class="box"><small>Week 2</small><b>${weekOneSummary.totalHours.toFixed(2)}h</b></div><div class="box"><small>Week 1</small><b>${weekTwoSummary.totalHours.toFixed(2)}h</b></div><div class="box"><small>Period Total</small><b>${reportPeriodSummary.totalHours.toFixed(2)}h</b></div><div class="box"><small>Regular</small><b>${reportPeriodSummary.regularHours.toFixed(2)}h</b></div><div class="box ${reportPeriodSummary.overtimeHours <= 0 ? "ot-zero" : ""}"><small>Overtime</small><b>${reportPeriodSummary.overtimeHours.toFixed(2)}h</b></div><div class="box"><small>Vacation</small><b>${reportPeriodSummary.vacationHours.toFixed(2)}h</b></div></section>${renderWeek(`Week 2`, `${displayShortDate(weekStart)} - ${displayShortDate(addDays(weekStart, 6))}`, reportWeekOne, weekOneSummary)}${renderWeek(`Week 1`, `${displayShortDate(addDays(weekStart, 7))} - ${displayShortDate(addDays(weekStart, 13))}`, reportWeekTwo, weekTwoSummary)}<p class="foot">Overtime is calculated per employee per week only after 40 worked hours. Vacation/PTO is tracked separately and does not create overtime.</p><p class="no-print">Use your browser print option and select Save as PDF.</p></main></body></html>`;
  ...
  function exportCsv(onlyApproved = false) {
    ...
    const rows = [
      ...
      [],
      ...makeRows(`Week 2: ${displayShortDate(weekStart)} - ${displayShortDate(addDays(weekStart, 6))}`, weekOneSource, summarizePayroll(weekOneSource)),
      ...makeRows(`Week 1: ${displayShortDate(addDays(weekStart, 7))} - ${displayShortDate(addDays(weekStart, 13))}`, weekTwoSource, summarizePayroll(weekTwoSource)),
    ];
                    ...
                    {[weekDates, weekTwoDates].map((datesForWeek, weekIndex) => (
                      <div key={weekIndex} className={cx(weekIndex > 0 && "mt-5")}>
                        <div className="week-strip mb-3 flex items-center justify-between rounded-[1.25rem] border border-slate-200/75 bg-slate-50/75 px-4 py-3 dark:border-white/10 dark:bg-white/[0.055]">
                          <p className="text-[11px] font-black uppercase tracking-[0.18em] text-cyan-700 dark:text-cyan-200">{weekIndex === 0 ? "Week 2" : "Week 1"}</p>
      ...
      <CardContent className="p-0">
        ...
        <div className="space-y-5 p-4 sm:p-5">
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            <DocumentationWeek title={`Week 2 • ${displayShortDate(weekStart)} – ${displayShortDate(addDays(weekStart, 6))}`} entries={weekOneEntries} getName={getName} openDayDetail={openDayDetail} />
            <DocumentationWeek title={`Week 1 • ${displayShortDate(addDays(weekStart, 7))} – ${displayShortDate(addDays(weekStart, 13))}`} entries={weekTwoEntries} getName={getName} openDayDetail={openDayDetail} />
          </div>
        </div>
      </CardContent>
    </Card>
